 ...
 */
function removeDuplicateLettersRecur(s) {
    function findFirstPick(s) {
        if (s.length == 0) {
            return s;
        }
        const charLastIndex = new Map();
...
function removeDuplicateLetters(s) {
    ...
    for (let i = 0; i < s.length; i++) {
        ...
        while (stack.length) {
            ...
            if (top > c) {
                const lastIndex = charLastIndex.get(top);
                if (lastIndex > i) {
                    stack.pop();
                    seened.delete(top);
                    continue;
...
Test_1.testWithResult(cases, removeDuplicateLettersRecur);
