...
function solveSudoku(board: string[][]): void {
    ...
    const base = (1 << 9) - 1;
    const bitsBoard:number[][] = new Array(m).fill(0).map(v=>new Array(n).fill(base));

    function setExcludeBits(i:number, j:number, testMask:number, mask:number){
    ...
    function setValue(i:number, j:number, num:number){
        //设置对应比特
