...
import { Case, testWithCheck, testWithResult } from "./Test";
...
function wiggleMaxLength(nums: number[]): number {
    ...
    let maxLen = 0;
    function helper(start: number, lt: boolean): number{
        let pre = nums[start];
        let max = 0;
...
function wiggleMaxLengthGreedy(nums: number[]): number {
    const n = nums.length;
    if (n < 2) { return n;}
...
testWithResult(cases, wiggleMaxLengthGreedy)