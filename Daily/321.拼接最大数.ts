...
function getMaxInArray(nums:number[], k:number):number[]{
...
function maxNumber(nums1: number[], nums2: number[], k: number): number[] {
    ...
    for(let i = 0; i <= k; i++){
        ...
        result = max(result, temp) as number[];
    }
    return result;
};
// @lc code=end

const cases:Case<[number[], number[], number], number[]>[] = [
...
testWithResult(cases, ([nums1, nums2, k])=>maxNumber(nums1, nums2, k), (a,b)=>{
